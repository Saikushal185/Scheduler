...
def algorithms(user: CurrentUser):
...
def generate(payload: GenerateScheduleRequest, db: DbSession, user: CurrentUser):
...
def runs(db: DbSession, user: CurrentUser, limit: int = 10):
...
def confirm(run_id: int, db: DbSession, user: CurrentUser,
...
def discard(run_id: int, db: DbSession, user: CurrentUser):
...
def list_constraints(db: DbSession, user: CurrentUser):
...
def create_constraint(payload: ConstraintCreate, db: DbSession, user: CurrentUser):
...
def delete_constraint(constraint_id: int, db: DbSession, user: CurrentUser):
    repo = ConstraintRepository(db)
    row = repo.get(constraint_id)
    if row is None:
        raise NotFoundError(f"Constraint {constraint_id} was not found")
