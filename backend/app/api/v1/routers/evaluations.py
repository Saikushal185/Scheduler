...
def list_metrics(db: DbSession, user: CurrentUser, active_only: bool = False):
...
def create_metric(payload: MetricCreate, db: DbSession, user: CurrentUser):
...
def set_weights(payload: MetricWeightsRequest, db: DbSession, user: CurrentUser):
    return [MetricRead.model_validate(m)
...
def list_evaluations(db: DbSession, user: CurrentUser, candidate_id: int | None = None,
...
def rankings(db: DbSession, user: CurrentUser, limit: int | None = None):
...
def profile(candidate_id: int, db: DbSession, user: CurrentUser):
    return EvaluationService(db).candidate_profile(candidate_id)


@router.get("/{evaluation_id}", response_model=EvaluationRead,
            summary="Get one evaluation")
def get_evaluation(evaluation_id: int, db: DbSession, user: CurrentUser):
    service = EvaluationService(db)
...
def create_evaluation(payload: EvaluationCreate, db: DbSession, user: CurrentUser):
    service = EvaluationService(db)
    return EvaluationRead.model_validate(service.serialise(service.create(payload)))
...
def update_evaluation(evaluation_id: int, payload: EvaluationUpdate, db: DbSession,
    ...
    return EvaluationRead.model_validate(
        service.serialise(service.update(evaluation_id, payload)))


@router.delete("/{evaluation_id}", response_model=Message,
               summary="Delete an evaluation")
def delete_evaluation(evaluation_id: int, db: DbSession, user: CurrentUser):
    EvaluationService(db).delete(evaluation_id)
