...
def list_panels(db: DbSession, user: StaffUser):
...
def panel_conflicts(db: DbSession, user: StaffUser):
...
def alternatives(payload: AlternativePanelRequest, db: DbSession, user: ManagerUser):
    return PanelService(db).alternatives(payload)


@router.get("/{panel_id}", response_model=PanelRead, summary="Get a panel group")
def get_panel(panel_id: int, db: DbSession, user: StaffUser):
...
def panel_availability(panel_id: int, db: DbSession, user: StaffUser,
...
def create_panel(payload: PanelCreate, db: DbSession, user: ManagerUser):
...
def update_panel(panel_id: int, payload: PanelUpdate, db: DbSession, user: ManagerUser):
...
def delete_panel(panel_id: int, db: DbSession, user: ManagerUser):
